...
describe("GcpAuthClient", () => {
	...
	it("fetches with token generated by CommandExecutor", async () => {
		...
		const mockExecutor = {
			execute: async (command: string, args: string[]) => {
				...
				expect(args).toEqual(["auth", "print-access-token"]);
				return "fake-token\n";
			}
		};
		...
		globalThis.fetch = async (url: RequestInfo | URL, init?: RequestInit) => {
			...
			fetchAuthHeader = headers.get("Authorization") || "";
			return new Response("ok", { status: 200 });
		};

		const client = new GcpAuthClient(mockExecutor);
		await client.fetchWithAuth("https://api.example.com", {}, "gcloud");

		expect(executeCalled).toBe(true);
		expect(fetchAuthHeader).toBe("Bearer fake-token");
	});

	it("re-uses token from cache if valid", async () => {
		const { GcpAuthClient } = __test__;
		...
		expect(fetchAuthHeader).toBe("Bearer cached-token");
	});

	it("retries with new token on 401 Unauthorized", async () => {
		const { GcpAuthClient } = __test__;
		...
		const mockExecutor = {
			execute: async () => {
				executeCallCount++;
				return executeCallCount === 1 ? "old-token\n" : "new-token\n";
			}
		};
		...
		const client = new GcpAuthClient(mockExecutor);
		const response = await client.fetchWithAuth("https://api.example.com", {}, "gcloud");
